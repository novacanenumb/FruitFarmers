...
contract FRUITSNode is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    ...
    function updateFRUITSAddress( address _FRUITSAddress) public onlyOwner{
        FRUITS = _FRUITSAddress;
    }
    ...
    function mint(uint _type) public payable {
        ...
        if (msg.sender != owner()) {
            require(IERC20(FRUITS).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(FRUITS).approve(address(uniswapV2Router), _tax);
            if (autosell) {
                swapTokensForEth(_tax);
            }
            IERC20(FRUITS).transfer(rewardPool, price-_tax);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(FRUITS).approve(address(uniswapV2Router), tokenAmount);
