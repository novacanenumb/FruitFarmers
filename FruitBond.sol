...
contract SINBondDepository is Ownable {
    ...
    IERC20 public immutable FRUIT; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints FRUIT when receives principle
    ...
    struct Bond {
        uint payout; // FRUIT remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 FRUIT ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) FRUIT
         */
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount, IStaking.LOCKUPS _lockup ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            FRUIT.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                FRUIT.approve( address(stakingHelper), _amount );
                ...
                FRUIT.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return FRUIT.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to FRUIT supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = FRUIT.totalSupply();
    ...
    /**
     *  @notice calculate amount of FRUIT available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or FRUIT) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != FRUIT, "NAT" );
