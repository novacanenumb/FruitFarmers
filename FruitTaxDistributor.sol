...
contract Ownable is OwnableData {
    ...
    function setFRUITSAddress(address _FRUITS) public {
    ...
    /// @notice Only allows the `owner` to execute the function.
    modifier onlyFRUITS() {
        require(msg.sender == FRUITS, "Ownable: caller is not the FRUITS");
        _;
    }
...
contract taxDistributor is Ownable {
    ...
    function addLiquidity(address urv2, address dai, uint256 tokenAmount, uint256 daiAmount, address liquidityTo) private {
        ...
        IERC20(FRUITS).approve(urv2, tokenAmount);
    ...
    function distribute(address urv2, address dai, address marketingWallet, uint256 daiForBuyback, address buybackWallet, uint256 liquidityTokens, uint256 daiForLiquidity, address liquidityTo) external onlyFRUITS{
