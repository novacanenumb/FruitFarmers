...
interface IsFRUIT is IERC20 {
    function rebase( uint256 FruitProfit_, uint epoch_) external returns (uint256);
...
contract FRUITStaking is Ownable {

    using SafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IsFRUIT;

    IERC20 public immutable FRUIT;
    IsFRUIT public immutable sFRUIT;
    ...
    constructor ( 
        ...
        FRUIT = IERC20(_FRUIT);
        require( _sFRUIT != address(0) );
        sFRUIT = IsFRUIT(_sFRUIT);
    ...
    /**
        @notice stake FRUIT to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient, LOCKUPS _lockup ) external returns ( bool ) {
        ...
        FRUIT.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sFRUIT.gonsForBalance( _amount ) ),
        ...
        if( _lockup == LOCKUPS.MONTH1 ) { // 1 - 1.25 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsFRUIT( sFRUIT ).gonsForBalance( _amount.mul(25).div(100) ),
                gonsAccount: IsFRUIT( sFRUIT ).gonsForBalance( _amount ),
        ...
        if( _lockup == LOCKUPS.MONTH3 ) { // 2 - 1.5 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsFRUIT( sFRUIT ).gonsForBalance( _amount.mul(50).div(100) ),
                gonsAccount: IsFRUIT( sFRUIT ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 150,
        ...
        if( _lockup == LOCKUPS.MONTH6 ) { // 3 - 2 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsFRUIT( sFRUIT ).gonsForBalance( _amount ),
                gonsAccount: IsFRUIT( sFRUIT ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 200,
        ...
        sFRUIT.safeTransfer( address(warmupContract), amountToTransfer );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve sFRUIT from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sFRUIT.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit sFRUIT in warmup and retrieve FRUIT
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = sFRUIT.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        FRUIT.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem sFRUIT for FRUIT
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        require(sFRUIT.balanceOf(msg.sender) - _amount >= sFRUIT.balanceForGons( lock.gonsAccount ), "Not enough sFRUIT for lockup");
        sFRUIT.safeTransferFrom( msg.sender, address(this), _amount );
        FRUIT.safeTransfer( msg.sender, _amount );
    ...
    function unstakeLocked(bool _trigger) external{
        ...
        if (lock.multiplier >0){
            sFRUIT.safeTransferFrom( msg.sender, address(this), sFRUIT.balanceForGons( lock.gonsAccount ));
            warmupContract.retrieve( address(this), sFRUIT.balanceForGons( lock.gonsWarmup ));

            if (lock.lockTimestamp <= block.timestamp){
                FRUIT.safeTransfer( msg.sender, sFRUIT.balanceForGons( lock.gonsWarmup + lock.gonsAccount ));
            }
            else{
                FRUIT.safeTransfer( msg.sender, lock.initialDeposit );
            }
    ...
    /**
        @notice returns the sFRUIT index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return sFRUIT.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sFRUIT.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sFRUIT.circulatingSupply();
    ...
    /**
        @notice returns contract FRUIT holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return FRUIT.balanceOf( address(this) ).add( totalBonus );
    }
